...
const AccordionTrigger = React.forwardRef(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="bg-[#F3F3F3] data-[state=open]:bg-white rounded-2xl">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "text-left text-[16px] leading-relaxed md:text-[20px] md:leading-normal transition-all p-5 pr-0 relative data-[state=closed]:before:content-[url(/accordianIconDown.svg)] data-[state=open]:before:content-[url(/accordianIconUp.svg)] before:absolute before:right-5 before:top-0 before:bottom-0 before:flex before:items-center before:-z-10 z-10 w-full",
        className
      )}
      {...props}>
        <span className="block w-[calc(100%-50px)]">{children}</span>
    </AccordionPrimitive.Trigger>
